...
public class SearchByWeatherActivity extends Activity {
	...
	String[] countries;
	int day=1;
	int mode = 1;	// 1 for Celsius, 2 for Fahrenheit
	String mode_identifier = "°C";
	Bitmap bm;
	
	/**
	 * Called when the activity is first created.
	 * It sets the display and adds a listener to all the buttons.
	 */
    @Override
    public void onCreate(Bundle savedInstanceState) {
        ...
        btn_switch.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
            	String button_text = btn_switch.getText().toString();
            	if (mode == 1) {
            		mode_identifier = "°F";
            		button_text = "°C";
            		mode = 2;
            	...
            	if ((edt_country.getText().toString().trim().length() > 0) && (edt_city.getText().toString().trim().length() > 0)) {
            		txt_output.setText(get_output_string());
            	} else {
            		txt_output.setText("");
            	}
            	btn_switch.setText(button_text);
            }
        });
		...
		edt_country.setOnItemClickListener(new OnItemClickListener() {  
			public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
				...
				if (country.contains(",")) {
					String first, second;
					first = country.substring(country.indexOf(",") + 2, country.length());
					second = country.substring(0, country.indexOf(","));
					edt_country.setText(first + " " + second);
				}		
				
				int index = GeographicalData.search(country);
				if (index != -1) {
    ...
    /**
     * Called when the back button is pressed, it clears the countries array.
     */
    ...
    private void set_icons() {
    	int condition_id;
    	int temperature = get_relevant_temp_in_celsius();
    ...
    /**
     * Retrieves the Celsius temperature for the set_icons function
     */
    private int get_relevant_temp_in_celsius() {
		if (mode == 1) {
			return relevant_temp;
		} else {
			return ReadWeather.castFarenheitToCelsius(Integer.toString(relevant_temp));
		}
	}
    ...
    private String get_output_string() {
		...
		if (ReadWeather.incompatible_location()) {
			output = "";
			Toast toast = Toast.makeText(this, "The location you requested doesn't exist or is spelled incorrectly.", Toast.LENGTH_LONG);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show(); 
			error = true;
		} else if(check_validity_of_results() == false) {
			output = "Sorry - there is a temporary disruption in the location you selected. Please try again later.";
			error = true;
		} else {
			ReadWeather.change_mode(mode);
    		
			set_relevant_temp();
			condition = ReadWeather.get_data(ReadWeather.get_key(2));
			
			output += "Temperature forcasted(" + mode_identifier + "): " + relevant_temp + "\n";
    ...
    private boolean check_validity_of_results() {
    	int size = ReadWeather.get_num_of_elements();
    	for (int i = 0; i < size; i++) {
    		if ((ReadWeather.get_data(ReadWeather.get_key(i)) == null) || (ReadWeather.get_data(ReadWeather.get_key(i)).length() == 0)) {
    			return false;
    		}
    	}
		return true;
