...
""" This method receives an user's email or id (as a unique user identifier) and
    returns the suitable UserStruct or None if no user with this email or id was found """
def get_user_by_email_or_id(email_or_id):
    query = db.GqlQuery("SELECT * FROM UserStruct WHERE email_or_id = :1", email_or_id);
    return query.get()

""" This method returns an random number which wasn't use before in ImageMetadataStruct
    random_num field"""
...
"""a structure to save whether we loaded the static data (images and details from the CSV file)"""     
